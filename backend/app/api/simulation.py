...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        prepared_statuses = ["ready", "running", "completed", "stopped"]
    ...
    try:
        ...
        try:
            ...
            if filtered_preview.filtered_count == 0:
                return jsonify({
                    "success": False,
                    "error": "No usable entities were found for the simulation. Build the graph first and ensure nodes have valid entity type labels, not only Entity."
                }), 400
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        logger.warning(f"Failed while looking up simulation {simulation_id} report lookup failed: {e}")
        return None
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to fetch profile in real time: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
    ...
    try:
        ...
        if os.path.exists(state_file):
            try:
                with open(state_file, 'r', encoding='utf-8') as f:
                    state_data = json.load(f)
                    simulation_status = state_data.get("status", "")
                    is_generating = simulation_status == "preparing"
                    config_generated = state_data.get("config_generated", False)
                    profiles_generated = state_data.get("profiles_generated", False)
                    entities_count = state_data.get("entities_count", 0)
                    error_message = state_data.get("error")
        ...
        failed = (
            ...
            or (not is_generating and not config_generated and not file_exists and entities_count == 0)
        )
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                logger.info(f"Simulation {simulation_id} preparation completed; status reset to ready (previous status: {state.status.value})")
