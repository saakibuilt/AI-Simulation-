...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts.append("\n### Interview Summary and Core Takeaways")
...
class ZepToolsService:
    ...
    def _generate_sub_queries(
        ...
        system_prompt = """You are an expert question analyst. Break a complex question into multiple sub-questions that can be independently observed in the simulated world.

Requirements:
1. Each sub-question should be specific enough to match observable agent behavior or events.
2. Cover different dimensions of the original question, such as who, what, why, how, when, and where.
...
Return the list of sub-questions in JSON format."""
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Based on your persona, all past memories, and prior actions, "
            ...
            "6. Give substantive answers with at least 2 to 3 sentences per question\n\n"
        )
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are an expert interview planner. Based on the interview requirement, choose the most suitable agents from the simulation-agent list.

Selection criteria:
1. The agent's identity or profession is relevant to the topic.
2. The agent may offer a distinct or valuable perspective.
3. Prefer a diverse set of viewpoints, such as supportive, opposing, neutral, and expert voices.
...
Select up to {max_agents} most suitable agents to interview and explain why."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are an expert journalist and interviewer. Generate 3 to 5 in-depth interview questions based on the interview need.
...
3. Cover facts, opinions, and feelings from multiple angles
4. Keep the phrasing natural, like a real interview
5. Keep each question concise
6. Ask directly without background framing or prefixes
...
Generate 3 to 5 interview questions."""
        ...
        try:
            ...
            return response.get("questions", [f"Regarding {interview_requirement}, what is your view?"])
            
        except Exception as e:
            logger.warning(t("console.generateInterviewQuestionsFailed", error=e))
            return [
                f"Regarding {interview_requirement}, what is your perspective?",
    ...
    def _generate_interview_summary(
        ...
        system_prompt = f"""You are a professional news editor. Based on multiple interview responses, write a concise interview summary.

Summary requirements:
1. Extract the main viewpoints from each side.
2. Highlight areas of consensus and disagreement.
3. Surface valuable quotations.
4. Stay objective and neutral.
5. Keep the total length under 1000 words.
