...
[Insight Forge]
Deep retrieval tool for section-level analysis.

Use it when you need:
- multi-angle investigation of a topic
- richer supporting evidence for a section
- relationship chains and entity-level insights
...
[Panorama Search]
Wide-angle retrieval for the overall event picture and evolution.

Use it when you need:
- a broad overview of the simulated world
...
[Quick Search]
Lightweight retrieval for direct fact lookup and verification.

Returns:
- the most relevant facts for a query"""

TOOL_DESC_INTERVIEW_AGENTS = """\
[Interview Agents]
Calls the live OASIS interview API to interview simulated agents across Twitter and Reddit.

Use it when you need:
- first-person reactions from different roles
- contrasting viewpoints
- vivid quotations for the report

Important:
- the OASIS simulation environment must be running"""



PLAN_SYSTEM_PROMPT = """\
You are writing a future-forecast report from a simulation world.

Core framing:
- The simulation requirement is the injected condition.
- The resulting agent behavior is a forecast of what may happen under those conditions.
- Focus on projected developments, reactions, risks, and emerging patterns.
- Do not write a present-day situation summary.

Task:
Produce a JSON outline for the report with:
- 2 to 5 sections
- a concise title
...
[Sample Forecast Facts]
{related_facts_json}

Design the most useful report structure for this forecast.
Requirements:
- 2 to 5 sections only
- keep it focused on the most important forecast findings"""
...
Rules:
1. Every claim must come from tool observations of the simulation world.
2. Do not rely on your own outside knowledge.
3. Each section must call tools at least 3 times and at most 5 times.
4. Use direct quotations from simulated agents when useful.
5. If tool output is in another language, translate it into the report language while preserving meaning.
6. If evidence is missing, say so plainly.

Formatting:
- Write body content only. Do not add headings with `#`, `##`, or `###`.
- The system adds the section title automatically.
- Use paragraphs, lists, block quotes, and bold text when helpful.
- Keep quotations as standalone blocks.

Available tools:
{tools_description}

Tool usage:
- In each reply, do exactly one of these:
  - Call one tool using:
    <tool_call>
    {{"name": "tool_name", "parameters": {{"key": "value"}}}}
    </tool_call>
  - Or provide final section content starting with `Final Answer:`
...
- gather evidence with tools before writing
- mix different tools when helpful
- do not add headings; write body text only

Start by deciding what information is still needed, then call a tool."""
...
- If you have enough evidence, respond with `Final Answer:`
- Otherwise call one more tool."""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "You have used only {tool_calls_count} tools, but at least {min_tool_calls} are required. "
    "Call another tool before giving `Final Answer:`. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Only {tool_calls_count} tools have been used so far. At least {min_tool_calls} are required. "
    "Call a tool to gather more simulation evidence. {unused_hint}"
)

REACT_TOOL_LIMIT_MSG = (
    "The tool-call limit has been reached ({tool_calls_count}/{max_tool_calls}). "
    'Respond now with `Final Answer:` using the evidence already collected.'
)

REACT_UNUSED_TOOLS_HINT = "\nHint: you have not used these tools yet: {unused_list}. Consider mixing tools for broader evidence."

REACT_FORCE_FINAL_MSG = "The tool-call limit has been reached. Respond directly with `Final Answer:` and write the section."



CHAT_SYSTEM_PROMPT_TEMPLATE = """\
You are a concise forecast assistant.
...
Rules:
1. Prefer answering from the report content first.
2. Answer directly and concisely.
3. Call tools only if the report is insufficient.
4. At most 1 to 2 tool calls are allowed in chat mode.
...
- explain briefly
- use block quotes when citing key evidence"""
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or need description (for example: 'Understand how students view the dorm formaldehyde issue')",
    ...
    def plan_outline(
        ...
        try:
            ...
            return ReportOutline(
                ...
                summary="Forecast of future trends and risks based on the simulation",
                sections=[
                    ReportSection(title="Forecast Scenario and Core Findings"),
                    ReportSection(title="Audience Behavior Forecast Analysis"),
