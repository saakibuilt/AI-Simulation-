...
logger = get_logger('analyticsfish.oasis_profile')
...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in social media persona generation. Create detailed and realistic personas for opinion simulation while staying as faithful as possible to the available real-world context. You must return valid JSON, and string values must not contain unescaped newline characters."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media persona for this entity and stay as faithful to the known real-world context as possible.
...
2. persona: detailed persona description as plain text, about 2000 words, covering:
   - basic profile (age, profession, education, location)
   - background (key experiences, relation to the event, social ties)
   - personality traits (MBTI, core temperament, emotional style)
   - social media behavior (posting frequency, content preferences, interaction style, language habits)
   - stance and opinions (attitude toward the topic, likely triggers and sympathies)
   - distinctive traits (catchphrases, unusual experiences, hobbies)
   - personal memory (an important part of the persona; explain how this individual relates to the event and what actions or reactions they have already had)
3. age: numeric age as an integer
...
Important:
- All field values must be strings, numbers, or arrays. Do not use newline characters inside values.
- persona must be one coherent block of text.
- {get_language_instruction()} The gender field must remain "male" or "female" in English.
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account persona for this organization or group and stay as faithful to the known real-world context as possible.
...
1. bio: official account bio, about 200 words, professional and appropriate
2. persona: detailed account profile as plain text, about 2000 words, covering:
   - organization basics (formal name, type, founding background, main functions)
   ...
   - publishing patterns (content types, posting frequency, active hours)
   - official stance (position on core topics and approach to controversy)
   - special notes (represented group profile and operating habits)
   - organizational memory (an important part of the persona; explain how the organization relates to the event and what actions or reactions it has already had)
3. age: always 30 as the account's virtual age
4. gender: always "other" for a non-person account
5. mbti: MBTI type describing the account style, such as ISTJ for rigorous and conservative
6. country: country name in English
7. profession: description of the organization's function
8. interested_topics: array of focus areas

Important:
- All field values must be strings, numbers, or arrays. Null values are not allowed.
- persona must be one coherent block of text without newline characters.
- {get_language_instruction()} The gender field must remain "other" in English.
- age must be the integer 30, and gender must be the string "other".
- The account voice must match the organization's role and identity."""
    ...
    def generate_profiles_from_entities(
        ...
        print(f"\n{'='*60}")
        print(f"Persona generation completed. Generated {len([p for p in profiles if p])} agents")
        print(f"{'='*60}\n")
