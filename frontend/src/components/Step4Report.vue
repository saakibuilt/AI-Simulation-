...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const factMatch = text.match(/Related Predictive Facts:\s*(\d+)/)
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        } else if (currentName && line.trim() && !line.match(/^Not selected|^Overall|^Final selection/)) {
    ...
    const summaryMatch = text.match(/### Interview Summary and Core Takeaways\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    if (queryMatch) result.query = queryMatch[1].trim()
    const countMatch = text.match(/Found\s*(\d+)\s*related items/)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      while ((match = numberedQuestionPattern.exec(answerText)) !== null) {
        matches.push({
